...
function SetDefaultStyle(node) {
	...
	if (node.data('grade') == "教育漢字 (kyōiku kanji)" || node.data('grade') == "常用漢字 (jōyō kanji)") {
			...
			$.getJSON('./resources/json/kanjidic.json', function(DATA) {
				...
				for (var i = 0; i < allNodes.length; i++) {
					for (var k = 0; k < typedchars.length; k++)  {
						if (elements.nodes[i].data.id.search(typedchars[k]) != -1 && expression != "/(?:)/i" && elements.nodes[i].data.unique != 1)
						{
							var meaningspaces = "";
							for (var g = elements.nodes[i].data.meaning.length - 1; g >= 0; g--) {       
								meaningspaces = meaningspaces.concat(elements.nodes[i].data.meaning[g]);  
								meaningspaces = meaningspaces.concat(", ");
							}
							meaningspaces = meaningspaces.slice(0, -2); 
							$('#RSLT').append('<li class="list-group-item link-class"><span class="text-muted">'+elements.nodes[i].data.id+
								' | </span><span class="text-muted">'+meaningspaces+'</span></li>');
						} 	
					}
					for (var j = 0; j < elements.nodes[i].data.meaning.length; j++) {
						if (elements.nodes[i].data.meaning[j].startsWith(searchField) == true && expression != "/(?:)/i" && expression.toString().length >= 6){
